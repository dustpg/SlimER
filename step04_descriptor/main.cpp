...
void update_cbuffer(void* coherent_buffer) {
    ...
    ++frame_id;
    const auto frame = float(frame_id) * 0.01f;

    BaseCBuffer cb{};

    cb.model = glm::rotate(glm::mat4(1.0f), frame * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    cb.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
...
void record(RHI::IRHICommandList* list, const pipe_t& pipe, RHI::IRHIImage* image, RHI::IRHIAttachment* attachment, int flight) {
    list->Begin();
    ...
    list->BeginRenderPass(rp);
    {
        ...
        RHI::PushDescriptorSetDesc descs[] = {  
            { pipe.baseCBuffers[flight], RHI::DESCRIPTOR_TYPE_CONST_BUFFER }
        };
...
int main() {
    ...
    std::thread render_thd{ [=, &flag, &swapchianImage, &swapchianAttachment]() {
        ...
        while (flag) {
            ...
            record(gra_lists[ctx.indexInFlight], pipe, swapchianImage[ctx.indexBuffer], swapchianAttachment[ctx.indexBuffer], ctx.indexInFlight);
