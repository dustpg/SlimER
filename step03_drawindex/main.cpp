...
pipe_t pipeline(RHI::API api,
    ...
    buildVertex(device, swapchian, bufferVertex.as_get());
    base_ptr<RHI::IRHIBuffer> bufferIndex;
    buildIndex(device, swapchian, bufferIndex.as_get());
    {
    ...
    return { pipeline, /*renderPass,*/ pipelineLayout, bufferVertex, bufferIndex };
}
